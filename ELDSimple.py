...
class window(Frame):
    def __init__(self):
        ...
        def delete_qg_text(self,e):
            global search_qg_text
            self.search_qg.delete(0, END)
            self.search_qg.insert(0, "")

        def delete_yt_text(self,e):
            global search_yt_text
            self.search_yt.delete(0, END)
            self.search_yt.insert(0, "")
        ...
        self.space3 = Label(self.lbl3, padx=1, pady=140)
        self.search_qg = Entry(self.lbl_g_query, width=50, font=("Arial 20"), bg="indian red", fg="goldenrod")
        self.search_qg.bind("<Button-1>", delete_qg_text)
        self.search_yt = Entry(self.lbl_yt_query, width=50, font=("Arial 20"), bg="indian red", fg="maroon")
        self.search_yt.bind("<Button-1>", lambda delete_yt_text: self.search_qg.delete(0, END), self.search_qg.insert(0, ""))
        ...
        self.search2 = Button(self.lbl_search2, text="Search", font=('Arial 12'), command=search_query, padx=1, pady=5)
...
def enter_query_qg(e):
    global url2
    if self.search_qg.get() == self.search_qg.get():
        url2 = url2 + self.search_qg.get()
        webbrowser.open(url2)

def enter_query_yt(e):
    global url
    if self.search_yt.get() == self.search_yt.get():
        url = url + self.search_yt.get()
        webbrowser.open(url)
